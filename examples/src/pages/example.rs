...
pub fn example() -> Html {
    ...
    let tabs_map = use_mut_ref(
        LinkedHashMap::<
            String,
            VChild<FunctionComponent<rust_web_server_yew::components::tabs::tab>>,
        >::new,
    );
            ...
            if map.contains_key(&tab) {
                ...
                *tabs_map.borrow_mut() = map;
                if let Some(t) = first_tab {
                    log!(format!("current tab is '{}' after delete {}", t, tab));
                    current.set(t);
                } else {
                    current.set("".into());
                }
...
fn get_menu(menu: String) -> VChild<FunctionComponent<rust_web_server_yew::components::tabs::tab>> {
    match menu.as_str() {
